...
def paginate_args(model):
    def decorator(func):
        ...
        def wrapper(*args, **kwargs):
            ...
            kwargs["sort_directions"] = sort_directions.split(",") if sort_directions else []
            nonlocal model
            if model is None:
            ...
            if request.endpoint == "users.items_per_user":
                del kwargs["page"]
                del kwargs["per_page"]
            for column, direction in zip(kwargs["sort_columns"], kwargs["sort_directions"]):
                try:
                    ...
                    mapping = {
                        ...
                        "database_credential": getattr(DatabaseCredential.app_name, direction),
                        "database_credentials": getattr(DatabaseCredential.app_name, direction),
...
def verify_editable(kwarg_name):
    def decorator(func):
        ...
        def wrapper(*args, **kwargs):
            ...
            if current_user and (
                current_user.is_admin or current_user.is_internal or current_object.check_owner(current_user)
            ):
                if current_object._sa_class_manager.class_ == User:
                    if current_object.is_internal and not current_user.is_internal:  # pragma: no cover
                        abort(403)
                elif current_object.owner.is_internal and current_user.is_internal != current_object.owner.is_internal:
                    abort(403)
            else:
                abort(403)
            return func(*args, **kwargs)
