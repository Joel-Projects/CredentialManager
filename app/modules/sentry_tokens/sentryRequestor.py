        ...
        for key, value in data.items():
            if isinstance(value, dict):
                ...
                if regex.match(value):
                    self.__setattr__(key, datetime.fromisoformat(value.strip('Z')).replace(tzinfo=timezone.utc).astimezone())
                else:
                    self.__setattr__(key, value)
            elif isinstance(value, list):
                self.__setattr__(key, [SentryResponse(i, key) if isinstance(i, dict) else i for i in value])
...
class SentryRequestor(object):
    ...
    def _request(self, method, url, itemName=None, params=None, data=None, json=None, raw=False):
