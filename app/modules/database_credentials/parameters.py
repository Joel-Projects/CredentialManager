...
class CreateDatabaseCredentialParameters(
    ...
    database_flavor = base_fields.String(
        default="postgres",
        required=True,
        description="Type of database, defaults to `postgres`",
    )
    database = base_fields.String(
        default="postgres",
        description="Working database to use, defaults to `postgres`",
    )
    database_host = base_fields.String(
        default="localhost",
        required=True,
        description="Database server address, defaults to `localhost`",
    )
    database_port = base_fields.Integer(
        default=5432,
        required=True,
