...
class DatabaseCredential(db.Model, Timestamp, InfoAttrs, StrName):
    ...
    database_host = db.Column(
        ...
        info={
            "label": "Database Host",
            "description": "Hostname of the server for the database. Defaults to 'localhost'. Can be set in user defaults.",
        },
    )
    database_port = db.Column(
        db.Integer,
        nullable=False,
        default=5432,
        info={
    ...
    database = db.Column(
        ...
        info={
            ...
            "description": "Main database to use. Defaults to 'postgres'.",
        },
    )
    use_ssh = db.Column(
        db.Boolean,
        ...
        info={
            "label": "SSH?",
            "description": "Check this to use a SSH tunnel to connect to the database server.",
        },
    )
    ...
    ssh_port = db.Column(
        ...
        info={
            "label": "SSH Port",
            "description": "Port for the SSH tunnel. Defaults to '22'",
        },
    )
    ssh_username = db.Column(
        db.String,
        info={
            "label": "SSH Username",
            "description": "Username to use for the SSH tunnel. Required if not using a SSH key.",
        },
    )
    ssh_password = db.Column(
        db.String,
        info={
            "label": "SSH Password",
            "description": "Password to use for the SSH tunnel. Required if not using a SSH key.",
        },
    )
    use_ssh_key = db.Column(
        db.Boolean,
        default=False,
        server_default="false",
        info={
    ...
    private_key_passphrase = db.Column(
        ...
        info={
            ...
            "description": "Passphrase to decrypt SSH token. Leave blank if no password is needed",
        },
    )
