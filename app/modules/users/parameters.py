...
class CreateUserParameters(PostFormParameters, schemas.BaseUserSchema):
    ...
    is_regular_user = base_fields.Boolean(dump_only=True ,default=True)
    is_internal = base_fields.Boolean(dump_only=True ,default=False)
...
class PatchUserDetailsParameters(PatchJSONParameters):
    ...
    """
    fields = ('current_password', User.password.key, User.is_active.fget.__name__, User.is_regular_user.fget.__name__, User.is_admin.fget.__name__,)
    ...
    def replace(cls, obj, field, value, state):
        ...
        if field in {User.is_active.fget.__name__, User.is_regular_user.fget.__name__}:
            with permissions.SupervisorRolePermission(obj=obj, password_required=True, password=state['current_password']):
                # Access granted
                pass
