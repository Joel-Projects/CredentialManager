...
def test_deleting_user(flask_app_client, login_as, regular_user_user_verification):
    response = flask_app_client.delete(f"/api/v1/user_verifications/{regular_user_user_verification.id}")
...
def test_deleting_self(flask_app_client, admin_user_instance, regular_user_user_verification):
    response = flask_app_client.delete(f"/api/v1/user_verifications/{regular_user_user_verification.id}")
    assert_success(
        response,
        None,
