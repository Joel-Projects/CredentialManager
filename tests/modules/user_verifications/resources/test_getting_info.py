...
def test_getting_list_of_user_verifications_by_authorized_user(flask_app_client, regular_user, regularUserUserVerification, adminUserUserVerification):
    ...
    assert response.json[0]['user_id'] == regularUserUserVerification.user_id
...
def test_getting_user_verification_info(flask_app_client, loginAs, regularUserUserVerification):
    ...
    if loginAs.is_admin or loginAs.is_internal:
        ...
        assert response.json['user_id'] == regularUserUserVerification.user_id
    else:
        assert403(response)

def test_getting_redditor_from_user_id(flask_app_client, regularUserInstance, adminUserUserVerification):
...
def test_getting_redditor_from_non_existant_redditor_with_bad_reddit_app(flask_app_client, regularUserInstance, adminUserUserVerification, redditApp):
    adminUserUserVerification.reddit_app = redditApp
    response = flask_app_client.post('/api/v1/user_verifications/get_redditor', data={'user_id': '123456789012345679', 'reddit_app_id': 2})

    assert response.status_code == 404

def test_getting_redditor_from_non_existant_user_id(flask_app_client, regularUserInstance):
    response = flask_app_client.post('/api/v1/user_verifications/get_redditor', data={'user_id': '123456789012345678'})
