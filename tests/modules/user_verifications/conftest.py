...
def regularUserUserVerification(temp_db_instance_helper, regular_user, redditApp):
    for _ in temp_db_instance_helper(UserVerification(reddit_app=redditApp, owner=regular_user, user_id='123456789012345678')):
        yield _

@pytest.fixture()
def adminUserUserVerification(temp_db_instance_helper, admin_user, redditApp):
    for _ in temp_db_instance_helper(UserVerification(reddit_app=redditApp, owner=admin_user, user_id='123456789012345679')):
        yield _

@pytest.fixture()
def internalUserUserVerification(temp_db_instance_helper, internal_user, redditApp):
    for _ in temp_db_instance_helper(UserVerification(reddit_app=redditApp, owner=internal_user, user_id='123456789012345670')):
        yield _
