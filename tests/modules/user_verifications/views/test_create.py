...
def test_create_user_verification(flask_app_client, login_as, reddit_app):
    with captured_templates(flask_app_client.application) as templates:
        ...
        user_verification = UserVerification.query.filter_by(user_id="123456789012345678").first()
        assert_created(user_verification, data)


@pytest.mark.parametrize("login_as", users, ids=labels)
def test_create_user_verification_with_extra_data(flask_app_client, login_as, reddit_app):
    with captured_templates(flask_app_client.application) as templates:
        response = flask_app_client.post(
            "/user_verifications",
            content_type="application/x-www-form-urlencoded",
...
@pytest.mark.parametrize("login_as", users, ids=labels)
def test_create_user_verification_other_user(flask_app_client, login_as, regular_user, reddit_app):
    with captured_templates(flask_app_client.application) as templates:
...
def test_create_user_verification_empty_extra_data_profile(flask_app_client, regular_user_instance):
    with captured_templates(flask_app_client.application) as templates:
