...
def test_User_static_roles_setting(
    ...
    if is_internal:
        user_instance.setStaticRole(user_instance.StaticRoles.INTERNAL)
    else:
        user_instance.unsetStaticRole(user_instance.StaticRoles.INTERNAL)

    if is_admin:
        user_instance.setStaticRole(user_instance.StaticRoles.ADMIN)
    else:
        user_instance.unsetStaticRole(user_instance.StaticRoles.ADMIN)

    if is_regular_user:
        user_instance.setStaticRole(user_instance.StaticRoles.REGULAR_USER)
    else:
        user_instance.unsetStaticRole(user_instance.StaticRoles.REGULAR_USER)

    if is_active:
        user_instance.setStaticRole(user_instance.StaticRoles.ACTIVE)
    else:
        user_instance.unsetStaticRole(user_instance.StaticRoles.ACTIVE)

    assert user_instance.hasStaticRole(user_instance.StaticRoles.INTERNAL) is is_internal
    assert user_instance.hasStaticRole(user_instance.StaticRoles.ADMIN) is is_admin
    assert user_instance.hasStaticRole(user_instance.StaticRoles.REGULAR_USER) is is_regular_user
    assert user_instance.hasStaticRole(user_instance.StaticRoles.ACTIVE) is is_active
