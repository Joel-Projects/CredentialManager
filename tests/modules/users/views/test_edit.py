...
def test_user_detail_edit_without_update_password(
    flask_app_client, db, regularUserInstance
):
    data = {
        "itemType": "users",
...
def test_user_detail_username(flask_app_client, loginAs, regular_user):
    ...
    with captured_templates(flask_app_client.application) as templates:
        ...
        if loginAs.is_admin or loginAs.is_internal:
            assert202(response)
            assertMessageFlashed(
                templates, "User 'regular_userNew' saved successfully!", "success"
            )
...
def test_user_detail_self_username(flask_app_client, loginAs):
    ...
    with captured_templates(flask_app_client.application) as templates:
        ...
        assertMessageFlashed(
            templates, "User 'newUsername' saved successfully!", "success"
        )
...
def test_user_detail_conflicting_username(flask_app_client, loginAs, regular_user):
    ...
    with captured_templates(flask_app_client.application) as templates:
        ...
        assertRenderedTemplate(templates, "edit_user.html")
        assert (
            templates["templates"][0][1]["usersForm"].errors["username"][0]
            == "Already exists."
        )
