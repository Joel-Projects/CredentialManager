...
def filterAccessToken(interaction, current_cassette):  # pragma: no cover
    ...
    try:
        ...
        return
    accessTokenPlaceholder = betamax.cassette.cassette.Placeholder(placeholder='<ACCESS_TOKEN>', replace=accessToken)
...
def filterRefreshToken(interaction, current_cassette):  # pragma: no cover
    ...
    try:
        ...
        return
    refreshTokenPlaceholder = betamax.cassette.cassette.Placeholder(placeholder='<REFRESH_TOKEN>', replace=refreshToken)
...
placeholders = {x: env_default(x) for x in
                ['auth_code', 'client_id', 'client_secret', 'password', 'redirect_uri', 'test_subreddit', 'user_agent',
...
test_data = {
    ...
    'scopes': ['edit', 'vote', 'modwiki', 'modflair', 'creddits', 'modconfig', 'history', 'modtraffic', 'modmail',
               'modposts', 'read', 'identity', 'report', 'privatemessages', 'modothers', 'modlog', 'structuredstyles',
               'wikiedit', 'subscribe', 'modself', 'save', 'submit', 'flair', 'wikiread', 'account', 'mysubreddits',
               'livemanage', 'modcontributors']
    }

@pytest.fixture()
def regularUserRefreshToken(temp_db_instance_helper, redditApp, regular_user):
    redditApp.owner = regular_user
    for _ in temp_db_instance_helper(RefreshToken(redditor='regularRedditor', refresh_token='regular', reddit_app=redditApp, owner=regular_user)):
        yield _

@pytest.fixture()
def adminUserRefreshToken(temp_db_instance_helper, redditApp, admin_user):
    redditApp.owner = admin_user
    for _ in temp_db_instance_helper(RefreshToken(redditor='adminRedditor', refresh_token='admin', reddit_app=redditApp, owner=admin_user)):
        yield _

@pytest.fixture()
def internalUserRefreshToken(temp_db_instance_helper, redditApp, internal_user):
    redditApp.owner = internal_user
    for _ in temp_db_instance_helper(RefreshToken(redditor='internalRedditor', refresh_token='internal', reddit_app=redditApp, owner=internal_user)):
        yield _
...
def redditApp(temp_db_instance_helper, regular_user):
    for _ in temp_db_instance_helper(RedditApp(app_name='regular_user_reddit_app', owner=regular_user, **redditAppData)):
        yield _

@pytest.fixture()
def regularUserUserVerification(temp_db_instance_helper, regular_user, redditApp):
    for _ in temp_db_instance_helper(
            UserVerification(reddit_app=redditApp, owner=regular_user, user_id='123456789012345678')):
        yield _