...
def test_getting_list_of_refresh_tokens(flask_app_client, loginAs, regularUserRefreshToken, adminUserRefreshToken, internalUserRefreshToken):
...
@pytest.mark.parametrize('loginAs', users, ids=labels)
def test_getting_list_of_refresh_tokens_owner_id(flask_app_client, loginAs, regularUserRefreshToken, adminUserRefreshToken):
    ...
    if not (loginAs.is_admin or loginAs.is_internal):
        ...
        assert response.json[0]['refresh_token'] == regularUserRefreshToken.refresh_token

def test_getting_refresh_token_info_by_unauthorized_user_must_fail(flask_app_client, regularUserInstance, regularUserRefreshToken):
    response = flask_app_client.get(f'/api/v1/refresh_tokens/{regularUserRefreshToken.id}')
...
def test_getting_refresh_token_info_by_redditor(flask_app_client, loginAs, refreshToken):
    response = flask_app_client.post(f'/api/v1/refresh_tokens/by_redditor', data={'reddit_app_id': refreshToken.reddit_app.id, 'redditor': refreshToken.redditor})
...
@pytest.mark.parametrize('loginAs', users, ids=labels)
def test_getting_refresh_token_info_by_redditor_by_self(flask_app_client, loginAs, regularUserRefreshToken):
    regularUserRefreshToken.owner = loginAs
    regularUserRefreshToken.reddit_app.owner = loginAs
    response = flask_app_client.post(f'/api/v1/refresh_tokens/by_redditor', data={'reddit_app_id': regularUserRefreshToken.reddit_app.id, 'redditor': regularUserRefreshToken.redditor})
