...
def test_creating_database_credential(flask_app_client, loginAs, regular_user):
    ...
    if loginAs.is_admin or loginAs.is_internal:
        assertSuccess(response, regular_user, DatabaseCredential, DetailedDatabaseCredentialSchema)
    else:
...
def test_creating_database_credential_for_self(flask_app_client, regularUserInstance):
    ...
    assertSuccess(response, regularUserInstance, DatabaseCredential, DetailedDatabaseCredentialSchema)

def test_creating_database_credential_for_self_with_owner(flask_app_client, regularUserInstance):
...
def test_creating_database_credential_with_bad_name(flask_app_client, loginAs):
    ...
    assert422(response, DatabaseCredential, messageAttrs=[('app_name', ['Name must be greater than 3 characters long.'])])