...
def test_modifying_database_credential(
    flask_app_client, regularUserDatabaseCredential, loginAs
):
    ...
    if loginAs.is_admin or loginAs.is_internal:
        assertSuccess(
            response,
            regularUserDatabaseCredential.owner,
            DatabaseCredential,
...
def test_modifying_database_credential_by_self(
    flask_app_client, regularUserInstance, regularUserDatabaseCredential
):
...
def test_modifying_database_credential_info_with_invalid_format_must_fail(
    ...
    regularUserDatabaseCredential.owner = regularUserInstance
    response = flask_app_client.patch(
        f"/api/v1/database_credentials/{regularUserInstance.id}",
        content_type="application/json",
        data=json.dumps(
            [
                {
            ...
            [
                {
                    "op": "replace",
                    "path": "/app_name",
                    "value": adminUserDatabaseCredential.app_name,
                }
            ]
        ),
    )
