...
def test_create_database_credential(flask_app_client, loginAs):
    with captured_templates(flask_app_client.application) as templates:
        response = flask_app_client.post('/database_credentials', content_type='application/x-www-form-urlencoded', data=data)
        assert201(response)
        assertRenderedTemplate(templates, 'database_credentials.html')
        databaseCredential = DatabaseCredential.query.filter_by(app_name='database_credential').first()
        assertCreated(databaseCredential, data)

@pytest.mark.parametrize('loginAs', users, ids=labels)
def test_create_database_credential_profile(flask_app_client, loginAs):
    with captured_templates(flask_app_client.application) as templates:
        response = flask_app_client.post(f'/profile/database_credentials', content_type='application/x-www-form-urlencoded', data=data)
        assert201(response)
        assertRenderedTemplate(templates, 'database_credentials.html')
        databaseCredential = DatabaseCredential.query.filter_by(app_name='database_credential').first()
...
def test_create_database_credential_other_user(flask_app_client, loginAs, regular_user):
    with captured_templates(flask_app_client.application) as templates:
        response = flask_app_client.post('/database_credentials', content_type='application/x-www-form-urlencoded', data={'owner': regular_user.id, **data})
...
def test_create_database_credential_bad_params_profile(flask_app_client, regularUserInstance):
    data['app_name'] = 'da'
    response = flask_app_client.post('/profile/database_credentials', content_type='application/x-www-form-urlencoded', data=data)
    assert422(response)
    databaseCredential = DatabaseCredential.query.filter_by(app_name='database_credential').first()
    assert databaseCredential is None