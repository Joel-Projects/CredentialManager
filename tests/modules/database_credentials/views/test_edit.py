...
@pytest.mark.parametrize('databaseCredential', databaseCredentials, ids=databaseCredentialLabels)
def test_database_credential_detail_edit_for_other_user(flask_app_client, loginAs, databaseCredential):
    with captured_templates(flask_app_client.application) as templates:
        data['itemId'] = f'{databaseCredential.id}'
        response = flask_app_client.post(f'/database_credentials/{databaseCredential.id}', content_type='application/x-www-form-urlencoded', data=data)
        if databaseCredential.owner.is_internal and not loginAs.is_internal:
            assert403(response, templates)
            modifiedDatabaseCredential = DatabaseCredential.query.filter_by(id=databaseCredential.id).first()
            ...
            assertMessageFlashed(templates, "Database Credentials 'newName' saved successfully!", 'success')
            modifiedDatabaseCredential = DatabaseCredential.query.filter_by(id=databaseCredential.id).first()
...
@pytest.mark.parametrize('loginAs', users, ids=labels)
def test_database_credential_detail_edit(flask_app_client, loginAs, regularUserDatabaseCredential):
    data['itemId'] = f'{regularUserDatabaseCredential.id}'
    with captured_templates(flask_app_client.application) as templates:
        response = flask_app_client.post(f'/database_credentials/{regularUserDatabaseCredential.id}', content_type='application/x-www-form-urlencoded', data=data)
        if loginAs.is_admin or loginAs.is_internal:
            assert202(response)
            assertRenderedTemplate(templates, 'edit_database_credential.html')
            assertMessageFlashed(templates, "Database Credentials 'newName' saved successfully!", 'success')
            modifiedDatabaseCredential = DatabaseCredential.query.filter_by(id=regularUserDatabaseCredential.id).first()
...
@pytest.mark.parametrize('loginAs', users, ids=labels)
def test_database_credential_detail_edit_self(flask_app_client, db, loginAs, regularUserDatabaseCredential):
    data['itemId'] = f'{regularUserDatabaseCredential.id}'
    regularUserDatabaseCredential = changeOwner(db, loginAs, regularUserDatabaseCredential)
    with captured_templates(flask_app_client.application) as templates:
        response = flask_app_client.post(f'/database_credentials/{regularUserDatabaseCredential.id}', data=data)
        assert202(response)
        assertRenderedTemplate(templates, 'edit_database_credential.html')
        assertMessageFlashed(templates, "Database Credentials 'newName' saved successfully!", 'success')
        modifiedDatabaseCredential = DatabaseCredential.query.filter_by(id=regularUserDatabaseCredential.id).first()
...
def test_database_credential_detail_conflicting_app_name(flask_app_client, db, regularUserInstance, regularUserDatabaseCredential, adminUserDatabaseCredential):
    ...
    with captured_templates(flask_app_client.application) as templates:
        response = flask_app_client.post(f'/database_credentials/{toBeModified.id}', json=data)
        assert response.status_code == 422
        assert response.mimetype == 'text/html'
        assertRenderedTemplate(templates, 'edit_database_credential.html')
        assert templates['templates'][0][1]['form'].errors['app_name'][0] == 'Already exists.'
        modifiedDatabaseCredential = DatabaseCredential.query.filter_by(id=toBeModified.id).first()
