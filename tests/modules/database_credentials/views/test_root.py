...
def test_root(flask_app_client, loginAs):
    with captured_templates(flask_app_client.application) as templates:
        response = flask_app_client.get("/database_credentials")
        assert200(response)
...
def test_root_profile(flask_app_client, loginAs):
    with captured_templates(flask_app_client.application) as templates:
        response = flask_app_client.get("/profile/database_credentials")
        assert200(response)
...
def test_root_user_page(flask_app_client, loginAs, regular_user):
    with captured_templates(flask_app_client.application) as templates:
        response = flask_app_client.get(f"/u/{regular_user}/database_credentials")
        assert200(response)
