...
@pytest.mark.parametrize('path,status_code', (
    ('/api/v1/api_tokens/', 401),
    ('/api/v1/api_tokens/1', 401),
))
def test_api_tokens_options_unauthorized(path, status_code, flask_app_client):
...
def test_api_tokens_options_authorized(path, expected_allowed_methods, flask_app_client, regular_user, regularUserApiToken, adminUserApiToken):
