...
def test_api_token_detail_edit_for_other_user(flask_app_client, loginAs, apiToken):
    ...
    with captured_templates(flask_app_client.application) as templates:
        response = flask_app_client.post(f'/api_tokens/{apiToken.id}', content_type='application/x-www-form-urlencoded', data=data)
        if apiToken.owner.is_internal and not loginAs.is_internal:
            ...
            assertMessageFlashed(templates, "API Token 'newName' saved successfully!", 'success')
            modifiedApiToken = ApiToken.query.filter_by(id=apiToken.id).first()
...
def test_api_token_detail_edit(flask_app_client, loginAs, regularUserApiToken):
    ...
    with captured_templates(flask_app_client.application) as templates:
        response = flask_app_client.post(f'/api_tokens/{regularUserApiToken.id}', content_type='application/x-www-form-urlencoded', data=data)
        if loginAs.is_admin or loginAs.is_internal:
            ...
            assert403(response, templates)
            assert regularUserApiToken == ApiToken.query.filter_by(id=regularUserApiToken.id).first()
...
def test_api_token_detail_edit_self(flask_app_client, db, loginAs, regularUserApiToken):
    ...
    with captured_templates(flask_app_client.application) as templates:
        response = flask_app_client.post(f'/api_tokens/{regularUserApiToken.id}', data=data)
        assert202(response)
        assertRenderedTemplate(templates, 'edit_api_token.html')
        assertMessageFlashed(templates, "API Token 'newName' saved successfully!", 'success')
        modifiedApiToken = ApiToken.query.filter_by(id=regularUserApiToken.id).first()
...
def test_api_token_detail_conflicting_name(flask_app_client, db, regularUserInstance, regularUserApiToken, adminUserApiToken):
    ...
    with captured_templates(flask_app_client.application) as templates:
        ...
        assertRenderedTemplate(templates, 'edit_api_token.html')
        assert templates['templates'][0][1]['form'].errors['name'][0] == 'Already exists.'
        assert regularUserApiToken == ApiToken.query.filter_by(id=regularUserApiToken.id).first()