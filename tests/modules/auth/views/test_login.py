...
def test_login_bad_credentials(flask_app_client, regular_user):
    with captured_templates(flask_app_client.application) as templates:
        ...
        assertMessageFlashed(templates, 'Please check your login details and try again.', 'error')

@pytest.mark.parametrize('loginAs', users, ids=labels)
def test_login_deactivated_account(flask_app_client, loginAs):
    with captured_templates(flask_app_client.application) as templates:
        response = flask_app_client.post('/login', data={'username': loginAs.username, 'password': loginAs.password_secret})
        assert response.status_code == 403
        assertRenderedTemplate(templates, 'login.html')
...
def test_login_create_initial_user(flask_app_client):
    with captured_templates(flask_app_client.application) as templates:
        response = flask_app_client.post('/create_initial_user', data={'username': 'username', 'password': 'password'}, follow_redirects=True)
        assert response.status_code == 200
    ...
    with captured_templates(flask_app_client.application) as templates:
        ...
        assertRenderedTemplate(templates, 'login.html')
    with captured_templates(flask_app_client.application) as templates:
        response = flask_app_client.post('/login', data={'username': 'username', 'password': 'password'}, follow_redirects=True)
        assert response.status_code == 200
        assertRenderedTemplate(templates, 'dash.html')
