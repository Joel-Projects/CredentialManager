...
def test_creating_sentry_token(flask_app_client, loginAs, regular_user):
    response = flask_app_client.post(path, data={'app_name': 'testToken', 'owner_id': regular_user.id, 'dsn': 'https://1234@abcd.com/1'})
...
def test_creating_sentry_token_for_self(flask_app_client, regularUserInstance):
    response = flask_app_client.post(path, data={'app_name': 'testToken', 'dsn': 'https://1234@abcd.com/1'})
    assertSuccess(response, regularUserInstance, SentryToken, DetailedSentryTokenSchema)

def test_creating_sentry_token_for_self_with_owner(flask_app_client, regularUserInstance):
    response = flask_app_client.post(path, data={'app_name': 'testToken', 'owner_id': regularUserInstance.id, 'dsn': 'https://1234@abcd.com/1'})
    assertSuccess(response, regularUserInstance, SentryToken, DetailedSentryTokenSchema)

@pytest.mark.parametrize('loginAs', users, ids=labels)
def test_creating_sentry_token_with_bad_name(flask_app_client, loginAs):
    response = flask_app_client.post(path, data={'app_name': 'to', 'dsn': 'https://1234@abcd.com/1'})
    assert422(response, SentryToken, messageAttrs=[('app_name', ['Name must be greater than 3 characters long.'])])