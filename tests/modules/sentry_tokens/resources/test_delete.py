...
def test_deleting_user(flask_app_client, loginAs, regularUserSentryToken):
    response = flask_app_client.delete(f'/api/v1/sentry_tokens/{regularUserSentryToken.id}')

    if loginAs.is_admin or loginAs.is_internal:
        assertSuccess(response, None, SentryToken, None, deleteItemId=regularUserSentryToken.id)
    else:
        assert403(response, SentryToken, oldItem=regularUserSentryToken, internal=True, action='deleted')
