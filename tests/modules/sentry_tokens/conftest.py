...
def regular_user_sentry_token(temp_db_instance_helper, regular_user):
    for _ in temp_db_instance_helper(
        SentryToken(app_name="regular_user_sentry_token", owner=regular_user, **test_data)
    ):
        yield _
...
def admin_user_sentry_token(temp_db_instance_helper, admin_user):
    for _ in temp_db_instance_helper(SentryToken(app_name="admin_user_sentry_token", owner=admin_user, **test_data)):
        yield _
...
def internal_user_sentry_token(temp_db_instance_helper, internal_user):
    for _ in temp_db_instance_helper(
        SentryToken(app_name="internal_user_sentry_token", owner=internal_user, **test_data)
    ):
        yield _
