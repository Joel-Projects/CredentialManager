...
def test_sentry_token_detail(flask_app_client, login_as, regular_user_sentry_token):
    with captured_templates(flask_app_client.application) as templates:
        response = flask_app_client.get(f"/sentry_tokens/{regular_user_sentry_token.id}")
...
@pytest.mark.parametrize("login_as", users, ids=labels)
def test_sentry_token_detail_self(flask_app_client, db, login_as, regular_user_sentry_token):
