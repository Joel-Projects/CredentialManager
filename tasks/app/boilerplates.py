...
def crud_module(context, module_name='', module_name_singular=''):
    ...
    if not re.match('^[a-zA-Z0-9_]+$', module_name):
        log.critical('Module module_name is allowed to contain only letters, numbers and underscores ([a-zA-Z0-9_]+)')
        return
    ...
    for template_file in ('__init__', 'models', 'parameters', 'resources', 'schemas'):
